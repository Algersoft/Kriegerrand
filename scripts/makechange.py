...
Open the wallets with krieger-api like so:

./krieger-api -w walletA.wallet -p yourpass --rpc-password test --bind-port 8070
./krieger-api -w walletB.wallet -p yourpass --rpc-password test --bind-port 8071
...
moveDecimal = 100  # KREGR has 6 decimals so 1000000 is the divide/multiply factor
minAmount = 100 * moveDecimal  # min number for amount to xfer
maxAmount = 5000 * moveDecimal  # max number for amount to xfer
anonymity = 3
fee = 10 # atomic units, KREGR would be 1000 as the tx network fee
