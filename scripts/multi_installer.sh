...
# Kriegerrand Multi-installer
# a one line clone-and-compile for kriegerrand:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/Algersoft/Kriegerrand/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/kriegerrand ]; then
            read -r -p "${1:-kriegerrand directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old kriegerrand directory" && echo
                    rm -rf "$PWD"/kriegerrand
                    ;;
                *)
                    _fail "kriegerrand directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir kriegerrand
        git clone -b master -q https://github.com/Algersoft/Kriegerrand kriegerrand   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd kriegerrand
    fi
}

_build_kriegerrand() {
    _note "Building kriegerrand from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/kriegerrand')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/kriegerrand')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Kriegerrand Multi_Installer v1.0 (pigly)"
...
_note "Look in 'kriegerrand/build/src/' for the executible binaries. See 'https://github.com/Algersoft/Kriegerrand' for more project support."